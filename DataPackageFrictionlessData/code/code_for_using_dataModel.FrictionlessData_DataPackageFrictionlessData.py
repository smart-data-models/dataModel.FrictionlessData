...
resources = [{'path': 'data/cpi.csv', 'name': 'cpi', 'profile': 'tabular-data-resource', 'schema': {'fields': [{'name': 'Country Name', 'type': 'string'}, {'name': 'Country Code', 'type': 'string'}, {'name': 'Year', 'type': 'year'}, {'name': 'CPI', 'description': 'CPI (where 2005=100)', 'type': 'number'}]}}]
attribute = "resources"
value = resources
# The next line creates the query for inserting this attribute in a NGSI-LD context broker if the attribute does not exist it creates it
print(sdm.update_broker(dataModel, subject, attribute, value, serverUrl=serverUrl, updateThenCreate=True))

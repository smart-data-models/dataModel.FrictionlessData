...
- `alternateName`: An alternative name for this item  - `caseSensitiveHeader`: Case Sensitive Header. Use of case in source CSV files is not always an intentional decision. For example, should 'CAT' and 'Cat' be considered to have the same meaning. Specifies if the case of headers is meaningful  - `commentChar`: Comment Character. Specifies that any row beginning with this one-character string, without preceding whitespace, causes the entire line to be ignored  - `csvddfVersion`: CSV Dialect schema version. A number to indicate the schema version of CSV Dialect. Version 1.0 was named CSV Dialect Description Format and used different field names  - `dataProvider`: A sequence of characters identifying the provider of the harmonised data entity.  - `dateCreated`: Entity creation timestamp. This will usually be allocated by the storage platform.  - `dateModified`: Timestamp of the last modification of the entity. This will usually be allocated by the storage platform.  - `delimiter`: Delimiter. A character sequence to use as the field separator  - `description`: A description of this item  - `doubleQuote`: Double Quote. If Double Quote is set to true, two consecutive quotes must be interpreted as one. Specifies the handling of quotes inside fields  - `escapeChar`: Escape Character. Specifies a one-character string to use as the escape character  - `header`: Header. Specifies if the file includes a header row, always as the first row in the file  - `id`: Unique identifier of the entity  - `lineTerminator`: Line Terminator. Specifies the character sequence that must be used to terminate rows  - `name`: The name of this item.  - `nullSequence`: Null Sequence. Specifies the null sequence, for example, \ and then 'N'  - `owner`: A List containing a JSON encoded sequence of characters referencing the unique Ids of the owner(s)  - `quoteChar`: Quote Character. Specifies a one-character string to use as the quoting character  - `seeAlso`: list of uri pointing to additional resources about the item  - `skipInitialSpace`: Skip Initial Space. Specifies the interpretation of whitespace immediately following a delimiter. If false, whitespace immediately after a delimiter should be treated as part of the subsequent field  - `source`: A sequence of characters giving the original source of the entity data as a URL. Recommended to be the fully qualified domain name of the source provider, or the URL to the source object.  - `type`: It has to be CSVDialectFrictionlessData. NGSI entity type    
